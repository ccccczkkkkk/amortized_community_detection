...
def main():
    ...
    for i in range(n_data):
        ...
        # Calculate Q of acp method and Ground Truth
        G = nx.from_numpy_array(adj_matrix_np)
        communities_pred = [set(np.where(predicted == c)[0]) for c in np.unique(predicted)]
        Q_pred = modularity(G, communities_pred)
        communities_gnd = [set(np.where(labels.cpu().numpy() == c)[0]) for c in np.unique(labels.cpu().numpy())]
        Q_gnd = modularity(G, communities_gnd)
        ...
        # Louvain method
        partition = community_louvain.best_partition(G)  # dict: node_id -> community_id
        pred_louvain = np.array([partition[n] for n in range(N)], dtype=int)

        # calculate AMI and Q of Louvain method
        communities_lou = [set(np.where(pred_louvain == c)[0]) for c in np.unique(pred_louvain)]
        Q_lou = modularity(G, communities_lou)
        ami_lou = adjusted_mutual_info_score(labels_np, pred_louvain)

        # plot adjacent matrix of Louvain
        pred_lou_sorted = remap_labels_by_cluster_size(torch.from_numpy(pred_louvain)).numpy()
        title_lou = f"Louvain (i={i}, N={N}, AMI={ami_lou:.3f}, Q_gnd={Q_gnd:.3f}, Q_lou={Q_lou:.3f})"
        plot_colored_adj_matrix_with_prediction(
